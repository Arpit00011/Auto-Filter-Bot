...
async def start(client, message):
    ...
    if data.startswith("sendfiles"):
        ...
        filesarr = []
        for file in files:
            nam_file_id = file.file_id
            k = await temp.BOT.send_cached_media(chat_id=PUBLIC_FILE_CHANNEL, file_id=nam_file_id)
            nam = await client.get_messages(PUBLIC_FILE_CHANNEL, k.id)
            mg = getattr(nam, nam.media.value)
            file_id = mg.file_id
            files_ = await get_file_details(nam_file_id)
            files1 = files_[0]
            title = '@DigitalGalaxyHQ  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files1.file_name.split()))
            size=get_size(files1.file_size)
            f_caption=files1.caption
            if f_caption is None:
                f_caption = f"@DigitalGalaxyHQ  {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files1.file_name.split()))}"
            if cd["update_channel_link"] != None:
                up = cd["update_channel_link"]
                button = [[
...
async def settings(client, message):
    ...
    try:
        ...
        link = 'https://t.me/DigitalGalaxyHQ'
